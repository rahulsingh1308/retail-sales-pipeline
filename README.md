...
ADLS Gen2 - Bronze Layer (Raw CSV)
     ↓
Azure Data Factory (Copy Activity → Parquet)
     ↓
ADLS Gen2 - Silver Layer (Cleaned Parquet)
     ↓
PySpark + Delta Lake (Transformations)
     ↓
...
retail-pipeline/
├── notebooks/
│   └── silver_to_gold.ipynb   # PySpark transformations
├── data/
│   ├── silver/                # Parquet files from ADF
│   └── gold/                  # Delta tables
└── .gitignore
```
